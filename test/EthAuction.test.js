...
contract('IntegralAuctionEth', accounts => {
  ...
  describe('#open', async () => {
    ...
    it('errors if auction was not funded', async () => {
      try {
        await iac.open(constants.GOOD.PARTIAL_TX, 17, 100, constants.ADDR0, ETHER, {from: seller, value: 0});
      } catch (e) {
    ...
    it('errors if asset is not address0', async () => {
      try {
        await iac.open(constants.GOOD.PARTIAL_TX, 17, 100, manager, ETHER, {from: seller, value: 10 ** 18});
      } catch (e) {
    ...
    it('errors if value is not equal to message.value', async () => {
      try {
        await iac.open(constants.GOOD.PARTIAL_TX, 17, 100, constants.ADDR0, ETHER, {from: seller, value: 10 ** 11});
      } catch (e) {
  ...
  describe('#claim', async () => {
    ...
    it('transfers fee to manager', async () => {
        let managerBalance = new BN(await web3.eth.getBalance(manager), 10);
        let managerShare = new BN(ETHER.divn(400));
        assert(managerBalance.eq(managerStartBalance.add(managerShare)));
    });

    it('transfers bidder share to bidder', async () => {
        let bidderBalance = new BN(await web3.eth.getBalance(constants.GOOD.BIDDER), 10);
        let bidderShare = ETHER.sub(ETHER.divn(400));
